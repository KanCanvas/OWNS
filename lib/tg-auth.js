...
function assertEnteredPhoneMatchesVerified(enteredPhone, verifiedPhone) {
  const normalizedEntered = String(enteredPhone || "").trim();
  ...
  if (!phonesMatch(normalizedEntered, verifiedPhone)) {
    return {
      ok: false,
      error:
        "Номер на сайте не совпадает с номером из Telegram. Проверьте номер и получите новый код в боте.",
    };
  }
